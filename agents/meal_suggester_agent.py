...
if "suggestions" not in st.session_state:
    ...
    st.session_state.suggestions = suggester.get_category_suggestions()
...
for idx, (emoji, name) in enumerate(st.session_state.suggestions):
    if cols[idx % 4].button(f"{emoji} {name}"):
        ...
        st.session_state.meal = name  # ustawia jako meal np. "Sushi"
        suggester = MealSuggesterAgent()
        st.session_state.meals = suggester.get_general_suggestions(name)
        st.rerun()

if st.session_state.meals:
    cols = st.columns(4)
    for idx, (emoji, name) in enumerate(st.session_state.meals):
        if cols[idx % 4].button(f"{emoji} {name}"):
            st.session_state.clicked_category = name
            st.session_state.meal = name  # ustawia jako meal np. "Sushi"
            suggester = MealSuggesterAgent()
